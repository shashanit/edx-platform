...
urlpatterns += patterns(
    ...
    url(r'(?ix)^settings/grading/{}(/)?(?P<index>\d+)?$'.format(parsers.URL_RE_SOURCE), 'grading_handler'),
    url(r'(?ix)^xblock/{}/gradeas/(?P<assignment_type>\w+)?$'.format(parsers.URL_RE_SOURCE), 'assignment_type_update'),
)
