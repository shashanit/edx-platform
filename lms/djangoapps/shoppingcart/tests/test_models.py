...
class PurchaseReportTest(ModuleStoreTestCase):
    ...
    correct_csv = dedent("""
        Purchase Time,Order ID,Status,Quantity,Unit Cost,Total Cost,Currency,Description,Annotation
    ...
    def test_csv_report_no_annotation(self):
        """
        Fill in gap in test coverage.  csv_report_annotation for PaidCourseRegistration instance with no
        matching annotation
        """
        # delete the matching annotation
        self.annotation.delete()
        self.assertEqual(u"", self.reg.csv_report_annotation)
